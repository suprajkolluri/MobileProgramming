...
class AddController: UIViewController, UIPickerViewDelegate, UISearchBarDelegate {
    ...
    func saveMovie(){
        ...
        aMovie.setValue(filename, forKey: "filename")
        do{
            try mContext!.save()
    ...
    func searchBarTextDidEndEditing(searchBar: UISearchBar) {
        let aConnectRPC:MovieCollectionStub = MovieCollectionStub()
        aConnectRPC.getMovieInfoFromRPC(searchBar.text!, callback: { (res: String, err: String?) -> Void in
            if err != nil {
                NSLog(err!)
            }else{
                NSLog(res)
                if let data: NSData = res.dataUsingEncoding(NSUTF8StringEncoding){
                    do{
                        ...
                        let jsonString: NSDictionary  = (dict!["result"] as? NSDictionary)!
                        let data: NSData? = try NSJSONSerialization.dataWithJSONObject(jsonString, options: NSJSONWritingOptions.PrettyPrinted)
                        var movie:MovieDescription?
                        if let data = data {
                            let json = NSString(data: data, encoding: NSUTF8StringEncoding)
                            if let json = json {
                                movie = MovieDescription(jsonStr: json as String)
                            }
                        }
                        if(movie!.titleVar == "Unknown"){
                            self.callOMDB(searchBar)
                        } else{
                            self.titleF.text = movie?.titleVar
                            self.yearF.text = movie?.yearVar
                            self.ratedF.text = movie?.ratedVar
                            self.releasedF.text = movie?.releasedVar
                            self.runtimeF2.text = movie?.runtimeVar
                            self.actorsF.text = movie?.actorsVar
                            self.plotF.text = movie?.plotVar
                            self.posterF.text = movie?.posterVar
                            let genreAll = movie?.genreVar
                            ...
                            self.filename = movie?.filenameVar
                        }
    ...
    func callOMDB(searchBar: UISearchBar){
